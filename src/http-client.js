...
const privateCall = ({ apiKey, apiSecret, endpoints, getTime = defaultGetTime, pubCall }) => (
	...
	return (data && data.useServerTime
		...
		return sendResult(
			fetch(
				`${!path.includes('/fapi') ? endpoints.base : endpoints.futures}${path}${noData ? '' : makeQueryString(newData)
				}`,
				{
					method,
...
export default opts => {
	...
	return {
		...
		futuresUserCommissionRate: payload => privCall('/fapi/v1/commissionRate', payload),
		futuresSetHedgeMode: payload => privCall('/fapi/v1/positionSide/dual', payload, 'POST'),
	}
}
